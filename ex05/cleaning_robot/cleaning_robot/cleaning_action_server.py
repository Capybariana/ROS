...
class CleaningActionServer(Node):
    ...
    def __init__(self):
        ...
        # Параметры уборки
        self.cleaning_width = 0.5  # ширина "очистки" за один проход
    ...
    def execute_callback(self, goal_handle):
        ...
        if goal.task_type == "clean_square":
            ...
            total_distance, cleaned_points = self.clean_circle(
                goal.target_x, goal.target_y, goal.area_size, goal_handle, feedback_msg
            )
    ...
    def clean_circle(self, center_x, center_y, radius, goal_handle, feedback_msg):
        """Чистка круговой области"""
        total_distance = 0.0
        cleaned_points = 0

        # Двигаемся к границе круга
        start_x = center_x + radius
        start_y = center_y
        total_distance += self.move_to(start_x, start_y, goal_handle, feedback_msg)

        # Делаем круговое движение
        num_points = 36  # количество точек на окружности
        for i in range(num_points + 1):
            ...
            total_distance += self.move_to(
                target_x, target_y, goal_handle, feedback_msg
            )
            cleaned_points += 1

            # Обновляем feedback
            if goal_handle.is_active:
                progress = min(100, int(i / num_points * 100))
                feedback_msg.progress_percent = progress
                feedback_msg.current_cleaned_points = cleaned_points
                feedback_msg.current_x = self.pose.x
                feedback_msg.current_y = self.pose.y
